...
bool ModuleSceneKen::Start()
{
	...
	unsigned int background_music = App->audio->LoadFx("honda.ogg");
	if (!App->audio->PlayFx(background_music, INT_MAX)) {
		return false;
	}
...
update_status ModuleSceneKen::Update()
{
	...
	float background_movement = 1.5f;
	App->renderer->Blit(graphics, 0, 0, &background, background_movement); // sea and sky
	App->renderer->Blit(graphics, 560, 8, &(flag.GetCurrentFrame()), background_movement); // flag animation

	// TODO 3: Draw the ship. Be sure to tweak the speed.
	App->renderer->Blit(graphics, 0, 0 + boat_y_increment, &red_ship, background_movement); // sea and sky

	// TODO 6: Draw the girl. Make sure it follows the ship movement!
	App->renderer->Blit(graphics, 192, 104 + boat_y_increment, &(girl.GetCurrentFrame()), background_movement);
